...
public class MongodbKit {
    ...
    protected static Log logger = Log.getLog(MongodbKit.class);
    ...
    public static void init(MongoClient client, String database) {
        ...
        MongodbKit.defaultDb = client.getDB(database);

    }

    public static void updateFirst(String collectionName, Map<String, Object> q, Map<String, Object> o) {
        MongodbKit.getCollection(collectionName).findAndModify(toDBObject(q), toDBObject(o));
    }

    public static int removeAll(String collectionName) {
        return MongodbKit.getCollection(collectionName).remove(new BasicDBObject()).getN();
    }

    public static int remove(String collectionName, Map<String, Object> filter) {
        return MongodbKit.getCollection(collectionName).remove(toDBObject(filter)).getN();
    }
    ...
    public static int save(String collectionName, List<Record> records) {
        ...
        return MongodbKit.getCollection(collectionName).insert(objs).getN();

    }

    public static int save(String collectionName, Record record) {
        return MongodbKit.getCollection(collectionName).save(toDbObject(record)).getN();
    }

    public static Record findFirst(String collectionName) {
        return toRecord(MongodbKit.getCollection(collectionName).findOne());
    }
    ...
    public static Page<Record> paginate(String collection, int pageNumber, int pageSize, Map<String, Object> filter,
        ...
        DBCollection logs = MongodbKit.getCollection(collection);
